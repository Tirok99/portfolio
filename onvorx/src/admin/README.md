...
- **Data** — every screen uses `useSiteContentRaw()` and the `src/admin/actions.ts`
  ...
  explicit **Save** (so a write to Supabase never fires per keystroke).
...
Changes are reflected on the public site immediately (Approach B), optimistically
applied in memory and persisted to Supabase via `adminApi.*` → `/api/admin/*`.
