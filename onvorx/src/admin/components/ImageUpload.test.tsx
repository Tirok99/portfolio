...
describe('ImageUpload', () => {
  ...
  it('uploads the picked file and calls onChange with the returned url + path', async () => {
    ...
    render(<ImageUpload label="Image" folder="projects" value={EMPTY} onChange={onChange} onClear={vi.fn()} />)
  ...
  it('shows the error slot and does not call onChange when the upload fails', async () => {
    ...
    const user = userEvent.setup()
    const onChange = vi.fn()
    render(<ImageUpload label="Image" folder="services" value={EMPTY} onChange={onChange} onClear={vi.fn()} />)
    const png = new File([Uint8Array.from([137, 80, 78, 71])], 'x.png', { type: 'image/png' })
    await user.upload(screen.getByLabelText(/choose file/i), png)
  ...
  it('on Remove with a stored path: calls onClear and deletes the old object', async () => {
    const user = userEvent.setup()
    const onClear = vi.fn()
    render(
      <ImageUpload
    ...
    expect(adminApi.deleteImage).toHaveBeenCalledWith('P')
  })
